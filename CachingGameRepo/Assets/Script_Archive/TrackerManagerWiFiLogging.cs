...
{
	public class TrackerManagerWiFiLogging : MonoBehaviour
	{
        ...
        private const string LOG_TAG = "Wave.Essence.Tracker.TrackerManagerWiFiLogging";
        ...
        private static TrackerManagerWiFiLogging instance = null;
        // public static TrackerManager Instance { get { return instance; } }
        public static TrackerManagerWiFiLogging Instance { get { return instance; } }
		...
		private void Update()
		{
			...
			if (printIntervalLog)
			{
				...
				for (int i = 0; i < TrackerUtils.s_TrackerIds.Length; i++)
				{
					...
					DEBUG(sb);
					SendLog("[DEBUG] "+ sb); // Custom modified
				}
			}
		...
		{
			sb.Clear().Append("OnApplicationPause() ").Append(pause); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified

            UpdateFocusedTracker();
			sb.Clear().Append("Resume() Focused tracker: ").Append(m_FocusedTracker.Name()); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
			...
			if (!pause)
			{
				...
				{
					TrackerId tracker = TrackerUtils.s_TrackerIds[i];
					sb.Clear().Append("Resume() 1.check ").Append(tracker.Name()).Append(" connection."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckTrackerConnection(tracker);

					sb.Clear().Append("Resume() 2.check ").Append(tracker.Name()).Append(" role."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckTrackerRole(tracker);

					sb.Clear().Append("Resume() 3.check ").Append(tracker.Name()).Append(" capability."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckTrackerCapbility(tracker);

					sb.Clear().Append("Resume() 4.check ").Append(tracker.Name()).Append(" input capability."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckTrackerInputs(tracker);

					sb.Clear().Append("Resume() 5.check ").Append(tracker.Name()).Append(" button analog type."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckTrackerButtonAnalog(tracker);

					sb.Clear().Append("Resume() 6.check ").Append(tracker.Name()).Append(" buttons."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckAllTrackerButtons(tracker);

					sb.Clear().Append("Resume() 7.check ").Append(tracker.Name()).Append(" battery."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    CheckTrackerBattery(tracker);

					sb.Clear().Append("Resume() 8.check ").Append(tracker.Name()).Append(" name."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		{
			UpdateFocusedTracker();
			sb.Clear().Append("Start() Focused tracker: ").Append(m_FocusedTracker.Name()); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified

            if (GetTrackerStatus() != TrackerStatus.Available) { return; }
			for (int i = 0; i < TrackerUtils.s_TrackerIds.Length; i++)
			{
				TrackerId tracker = TrackerUtils.s_TrackerIds[i];
				sb.Clear().Append("Start() 1.check ").Append(tracker.Name()).Append(" connection."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckTrackerConnection(tracker);

				sb.Clear().Append("Start() 2.check ").Append(tracker.Name()).Append(" role."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckTrackerRole(tracker);

				sb.Clear().Append("Start() 3.check ").Append(tracker.Name()).Append(" capability."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckTrackerCapbility(tracker);

				// For WVR_TrackerCapabilities.supportsInputDevice
				sb.Clear().Append("Start() 4.check ").Append(tracker.Name()).Append(" input capability."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckTrackerInputs(tracker);

				// Depends on IsTrackerInputAvailable
				sb.Clear().Append("Start() 5.check ").Append(tracker.Name()).Append(" button analog type."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckTrackerButtonAnalog(tracker);

				// Depends on IsTrackerInputAvailable
				sb.Clear().Append("Start() 6.check ").Append(tracker.Name()).Append(" buttons."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckAllTrackerButtons(tracker);

				// For WVR_TrackerCapabilities.supportsBatteryLevel
				sb.Clear().Append("Start() 7.check ").Append(tracker.Name()).Append(" battery."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckTrackerBattery(tracker);

				sb.Clear().Append("Start() 8.check ").Append(tracker.Name()).Append(" name."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                UpdateTrackerDeviceName(tracker);
			}
		}
		#endregion
		...
		private void StartTrackerLock()
		{
			...
			sb.Clear().Append("StartTrackerLock() result: ").Append(result); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		private void StartTrackerThread()
		{
			...
			{
				sb.Clear().Append("StartTrackerThread()"); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                StartTrackerLock();
			}
		}
		...
		public void StartTracker()
		{
			...
			if (!CanStartTracker())
			{
				sb.Clear().Append("StartTracker() can NOT start tracker."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                if (trackerResultCB != null) { trackerResultCB = null; }
				return;
			}

			sb.Clear().Append("StartTracker() from ").Append(caller); INFO(sb); SendLog("[INFO] " + sb); // Custom modified
		...
		private void StopTrackerLock()
		{
			...
			sb.Clear().Append("StopTrackerLock()"); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		private void StopTrackerThread()
		{
			lock (trackerThreadLocker)
			{
				sb.Clear().Append("StopTrackerThread()"); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                StopTrackerLock();
			}
		}
		public void StopTracker()
		{
			...
			if (!CanStopTracker())
			{
				sb.Clear().Append("CanStopTracker() can NOT stop tracker."); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                return;
			}
		...
		private void CheckTrackerConnection(TrackerId trackerId)
		{
			...
			if (s_TrackerConnection[trackerId] != connected)
			{
				...
				sb.Clear().Append("CheckTrackerConnection() ").Append(trackerId.Name()).Append(": ").Append(s_TrackerConnection[trackerId]); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                CheckStatusWhenConnectionChanges(trackerId);
			}
		}
		void CheckStatusWhenConnectionChanges(TrackerId trackerId)
		{
			sb.Clear().Append("CheckStatusWhenConnectionChanges() ").Append(trackerId.Name()); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		private void OnTrackerConnected(WVR_Event_t systemEvent)
		{
			...
			sb.Clear().Append("OnTrackerConnected() ").Append(trackerId.Name()); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		{
			TrackerId trackerId = systemEvent.tracker.trackerId.Id();
			sb.Clear().Append("OnTrackerDisconnected() ").Append(trackerId); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		private void CheckTrackerButtonAnalog(TrackerId trackerId)
		{
			...
			DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
        }
		#endregion
		...
		private void UpdateFocusedTracker()
		{
			...
			for (int i = 0; i < TrackerUtils.s_TrackerIds.Length; i++)
			{
				if ((int)TrackerUtils.s_TrackerIds[i] == id)
				{
					...
					sb.Clear().Append("UpdateFocusedTracker() ").Append(m_FocusedTracker.Name()); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
                    return;
				}
			}
			sb.Clear().Append("UpdateFocusedTracker() WVR_GetFocusedTracker() invalid id ").Append(id); WARNING(sb); SendLog("[WARNING] " + sb); // Custom modified
        }
		#endregion
		...
		{
			sb.Clear().Append("SetFocusedTracker() ").Append(tracker.Name()); DEBUG(sb); SendLog("[DEBUG] " + sb); // Custom modified
		...
		/**
		 * Custom Modified to debug in VR after building process
		 */
        IEnumerator SendLog(string logMessage)
        {
            ...
            {
                UnityEngine.Debug.Log("Log sent successfully");
            }
        }
    }
}
