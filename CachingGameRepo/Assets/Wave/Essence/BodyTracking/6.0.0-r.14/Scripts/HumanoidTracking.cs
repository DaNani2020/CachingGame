...
namespace Wave.Essence.BodyTracking
{
	...
	public class HumanoidTracking : MonoBehaviour
	{
		...
		void WARNING(StringBuilder msg) { Log.w(LOG_TAG, msg, true); }
		...
		private ExtrinsicInfo_t m_LeftFoot = new ExtrinsicInfo_t(true, ExtrinsicVector4_t.identity);
		...
		private ExtrinsicInfo_t m_RightFoot = new ExtrinsicInfo_t(true, ExtrinsicVector4_t.identity);
		...
		public IEnumerator StartFixUpdateBodyTracking()
		{
			...
			if (!m_CustomizeExtrinsics)
			{
                ...
                sb.Clear().Append("Bodytracking RESULT: ").Append(result); DEBUG(sb);
            }
			else
			{
            ...
            sb.Clear().Append("StartFixUpdateBodyTracking() CreateBodyTracking result ").Append(result).Append(", id: ").Append(m_AvatarID); DEBUG(sb);
            if (result != BodyTrackingResult.SUCCESS) { yield break; }
